...
- CDN-first runtime refresh: when `VOCODER_CDN_URL` is configured the SDK
  fetches locale bundles directly from Cloudflare R2, bypassing the Vocoder
  API entirely. Uses `If-Modified-Since` for native 304 support at the CDN
  layer. Falls back to the API transparently on any CDN error.

- Background refresh now only fires when a locale has no build-time
  translations (i.e. was empty at build time). If virtual modules loaded
  successfully the CDN/API is never contacted on locale switches.
